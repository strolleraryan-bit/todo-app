...
## Deploying an update (important — service worker caching)

`sw.js` caches `index.html`, `css/style.css`, and `js/app.js` **cache-first**
for offline use, keyed to the `CACHE_NAME` constant at the top of the file.
As long as that name stays the same, a browser that already has the app
loaded (especially once installed as a home-screen PWA) will keep serving
the *old* cached JS/CSS forever, even after you push new files to
GitHub/Netlify — it never even checks the network for them.

**So every time `app.js`, `style.css`, or `index.html` changes, bump
`CACHE_NAME` in `sw.js` (e.g. `v5` → `v6`).** That forces the `activate`
handler to delete the old cache and re-fetch everything fresh. Forgetting
this step is the single most common reason "I deployed but nothing
changed" happens.

If someone is already stuck on an old cached version, a code deploy alone
won't fix it for them — they also need to either reopen the app after the
new service worker activates (usually automatic on next launch once
`CACHE_NAME` differs), or manually clear site data / unregister the service
worker for the site once as a one-time fix.
