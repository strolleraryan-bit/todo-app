(function(){
  ...
  let meta = { notificationsEnabled: false, customTags: [], theme: null }; // small persisted settings blob
  ...
  function mkTask(text, priority, due, done, note, tags, repeat, remind, serial){
    return {
      id: idCounter++, text, priority, due, done: !!done, note: note||"",
      tags: tags || [], subtasks: [], pinned:false, repeat: repeat || "none",
    ...
    try{
      ...
      if(result && result.value){
        ...
        if(parsed && Array.isArray(parsed.tasks)){
          tasks = parsed.tasks.map(t => ({ tags:[], subtasks:[], pinned:false, repeat:"none", note:"", remind:"", reminded:false, ...t }));
  ...
  addForm.addEventListener("submit", (e) => {
    ...
    // a reminder needs a date to anchor to — fall back to today if only a time was set
    const due = dueInput.value || (remindInput.value ? todayISO() : "");
    const t = mkTask(text, composerPriority, due, false, "", tags, repeatSelect.value, remindInput.value);
    tasks.unshift(t);
    taskInput.value = "";
  ...
  function toggleDone(id, viaSwipe){
    ...
    if(t.done){
      ...
      if(t.repeat && t.repeat !== "none"){
        const next = mkTask(t.text, t.priority, nextDueDate(t.due, t.repeat), false, "", [...t.tags], t.repeat, t.remind);
        tasks.push(next);
        pushUndo(
  ...
  parseGenerate.addEventListener("click", () => {
    ...
    const due = calendarMode ? selectedDate : "";
    const batchTags = parseSelectedTag ? [parseSelectedTag] : [];
    const serials = claimSerials(items.length);
    const created = items.map((text, i) => mkTask(text, "med", due, false, "", [...batchTags], "none", "", serials[i]));
